...
public class ElytraTask {
    ...
    private final static TimelinessCounter baritoneControlCounter = new TimelinessCounter(20);
    ...
    private static void FireworkChecker(@NotNull MinecraftClient client, BlockPos segPos) {
        ...
        if (slots <= 1) {
            ...
            if (replaceList.size() <= slots)
                throw new TaskException("无槽位放置烟花");
        }
        if (count < 64) {
            // 烟花数量少，从背包里拿一些
            ...
            for (int i = 9; i < 36; i++) {
                ...
                if (item == Items.FIREWORK_ROCKET) {
                    ...
                    c += stack.getCount();
                    int finalI = i;
                    runOnMainSync(() -> client.interactionManager.clickSlot(handler.syncId, finalI, 0,
                                                                            SlotActionType.QUICK_MOVE, client.player));
                }
            }
            runOnMainSync(handled::close);
     ...
     * @param client 客户端对象
     */
    private static void baritoneChecker(@NotNull MinecraftClient client, int x, int z) {
        if (client.player == null)
            throw new TaskException("player 为null");
        // baritone寻路失败，等待重置状态或auto log
        if (SegFailedCounter.getCount() > 25) {
            if (SegFailedCounter.getCount() > 30)
                throw new TaskException("baritone寻路异常");
            else if (waitReset) {
                runOnMainSync(() -> {
            ...
            if (LastPos != null && client.player.getBlockPos().isWithinDistance(LastPos, 25)) {
                ...
                if (spinTimes > 4)
                    ...
                    if (spinTimes > 1) {
                        ...
                        flyToOpen(client, x, z);
                        for (int i = 0; i < 40; i++) {
                            if (!client.player.getBlockPos().isWithinDistance(LastPos, 30))
                                break;
                            delay(1);
                        }
                    } else
    ...
    private static void AutoEating(@NotNull MinecraftClient client) {
        ...
        if (!client.player.isInLava() && client.player.getVelocity().length() > 1.3 &&
            ...
            if (slot2 == -1)
                throw new TaskException("没有足够的食物了！");
            int finalSlot = slot2;
            runOnMainSync(() -> {
            ...
            for (int i = 0; i < 35; i++) {
                ...
                if (client.player.getVelocity().length() < 0.7 || client.player.isInLava() ||
                    ...
                    if (client.interactionManager != null)
                        runOnMainSync(() -> client.interactionManager.stopUsingItem(client.player));
                    return;
                }
                delay(1);
            }
    ...
    private static void AutoEscapeLava(@NotNull MinecraftClient client) {
        ...
        if (inFireTick == -1) {
            ...
            if (client.player == null)
                return;
            if (client.player.isInLava())
                throw new TaskException("逃离岩浆失败！");
        }
    ...
    private static void flyToOpen(@NotNull MinecraftClient client, int x, int z) {
        ...
        for (int TakeTime = 0; TakeTime < 6; TakeTime++) {
            ...
            if (yh != 0) {
                ...
                if (t == null)
                    throw new TaskException("目标角度核实失败！");
            }
            ...
            while (!(client.player.getBlockPos().isWithinDistance(TakePos, 1.5) ||
                ...
                if (inLavaTicks.getCount() > 15)
                    throw new TaskException("玩家在飞往开阔地带的路上飞入岩浆");
                if (!client.player.isGliding()) {
                    List<BlockPos> bp1 = computeOnMain(() -> getPotentialJumpBlockingBlocks(1));
                    if (!bp1.isEmpty())
                        clearBlockingBlock(client, x, z, bp1);
                    elytraTakeoff(client);
                    nearGround = true;
                    break;
                }
    ...
    private static void clearBlockingBlock(@NotNull MinecraftClient client, int x, int z, @NotNull List<BlockPos> bp) {
        ...
        for (int i = 0; i < 200; i++) {
            ...
            if (!BaritoneAPI.getProvider().getPrimaryBaritone().getBuilderProcess().isActive() || bp2.isEmpty()) {
                ...
                runOnMainSync(() -> BaritoneAPI.getProvider().getPrimaryBaritone().getElytraProcess().pathTo(
                        new BlockPos(x, 0, z)));
                delay(10);
                return;
            }
            ...
            if (client.player.isGliding()) {
                ...
                if (!client.player.isOnGround()) {
                    ...
                    if (jumpingCounter.getCount() <= 3) {
                        elytraTakeoff(client);
                        for (int i = 0; i < 40; i++) {
                            if (BaritoneControlChecker.isControlPlayer() || client.player.isOnGround())
                                break;
                            delay(1);
                        }
    ...
    private static void WaitForLoadChunks(@NotNull MinecraftClient client, boolean verboseDisplayDebug) {
        ...
        if (a > 0.4 && getPotentialJumpBlockingBlocks(-7).isEmpty()) {
            msg.SendMsg(client.player, "未加载区块太多，暂停baritone等待加载，接下来可能出现视角剧烈晃动！请不要直视屏幕！",
                        MsgLevel.warning);
            runOnMainSync(() -> BaritoneAPI.getProvider().getPrimaryBaritone().getCommandManager().execute("p"));
            cameraMixinSwitch = true;
            int tick = currentTick;
            ...
            if ((currentTick - tick) % 2 == 1)
                runOnMainSync(() -> client.player.setYaw(client.player.getYaw() + 180));
            runOnMainSync(() -> BaritoneAPI.getProvider().getPrimaryBaritone().getCommandManager().execute("r"));
            ...
            if (Objects.equals(
                ...
                for (int i = 0; i < 600; i++) {
                    ...
                    if (Objects.equals(client.world.getBiome(client.player.getBlockPos()).getKey().map(
                        ...
                        runOnMainSync(() -> BaritoneAPI.getProvider().getPrimaryBaritone().getCommandManager().execute(
                                "stop"));
                        delay(5);
                        break;
                    }
                    if (i == 599)
                        throw new TaskException("无法从baritone鞘翅任务中结束！");
                    delay(1);
                }
                status = TaskStatus.REPAIR_ELYTRA;
                delay(1);
                ...
                // 找一个可以放附魔之瓶的快捷栏槽位
                int FireworkSlot = SupplyTask.findItemInHotBar(client.player, Items.FIREWORK_ROCKET);
                ...
                if (client.player.getInventory().getStack(FireworkSlot).getItem() != Items.EXPERIENCE_BOTTLE ||
                    client.player.getInventory().getStack(FireworkSlot).getCount() < 30)
                ...
                // 开启火球保护任务
                Thread fireballProtector = ModTaskManager.startThread(() -> {
                    while (true) {
                ...
                for (int i = 0; i < 40; i++) {
                    ...
                    if (client.player.getPitch() < 85) {
                        ModTaskManager.startThread(() -> {
                            delay(5);
                            runOnMainSync(() -> client.player.setPitch(90));
                        });
                        delay(6);
                    }
                    runOnMainSync(() -> client.interactionManager.interactItem(client.player, Hand.MAIN_HAND));
                    delay(4);
                }
                ...
                // 再次打开物品栏
                HandledScreen<?> handled2 = computeOnMain(() -> {
                    client.setScreen(new InventoryScreen(client.player));
                ...
                delay(15);
                status = TaskStatus.FLYING;
                return;
            }
            if (ElytraStack.getDamage() > ElytraStack.getMaxDamage() - 8)
                throw new TaskException("鞘翅耐久过低！");
        }
    }
